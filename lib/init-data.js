...
// v1.9.0: same reasoning and pattern as mergeRolePermissions() above -
// event-config.json already exists on any environment that's been running
// since before this release, so ensureFile() (a no-op once the file
// exists) never adds the 3 new keys (venue/activityStartDate/
// activityEndDate) needed by the Document Generator. This heals that on
// every startup, adding only what's missing - titleLine1/titleLine2/year
// (and any other existing key) are never touched or reset.
function mergeEventConfig() {
  const onDisk = store.readJSON(EVENT_CONFIG_FILE, {});
  const missingKeys = Object.keys(SEED_EVENT_CONFIG).filter((key) => !(key in onDisk));
  if (missingKeys.length === 0) return Promise.resolve();

  return store.update(EVENT_CONFIG_FILE, SEED_EVENT_CONFIG, (current) => {
    const stillMissing = Object.keys(SEED_EVENT_CONFIG).filter((key) => !(key in current));
    if (stillMissing.length === 0) return { data: current, result: null };
    const data = { ...current };
    stillMissing.forEach((key) => { data[key] = SEED_EVENT_CONFIG[key]; });
    return { data, result: stillMissing };
  }).then((addedKeys) => {
    if (addedKeys && addedKeys.length) {
      console.log(`[startup] event-config.json: auto-added missing key(s) not present in the existing file: ${addedKeys.join(', ')}`);
    }
  });
}
...
function initData() {
  ...
  // Independent files (store.js locks per-file), safe to merge concurrently.
  return Promise.all([mergeRolePermissions(), mergeEventConfig()]);
}
