...
// 1.4 safety patch: restore is a "break glass" action, off unless the
// operator explicitly starts the server with RESTORE_MODE=enabled. Backup
// creation, listing, and everything else in this file is unaffected -
// this only gates restoreBackup() below, so normal race-day operation never
// has restore silently available.
function isRestoreModeEnabled() {
  return process.env.RESTORE_MODE === 'enabled';
}
...
//
// `scope` must be the literal string 'active' - restore only ever has one
// target (the live data/*.json files, i.e. "the active event"), so this
// isn't selecting between options, it's making the caller state out loud
// what they're about to overwrite, matching routes/system.js's confirmation
// dialog text. Anything else is rejected, same as a missing confirm flag.
async function restoreBackup(timestamp, scope, actor) {
  if (!isRestoreModeEnabled()) {
    return { ok: false, error: 'Mod pemulihan dilumpuhkan - mulakan semula pelayan dengan RESTORE_MODE=enabled untuk membenarkannya' };
  }
  if (scope !== 'active') {
    return { ok: false, error: 'Skop pemulihan mesti disahkan sebagai "active"' };
  }
  // Reject anything that isn't a plain positive integer *before* it ever
  // touches a path - our own backup dir names are always Date.now(), so a
  // legitimate timestamp never needs to be anything else. This closes off
  // path traversal via the URL param (e.g. "../../etc") outright, rather
  // than relying only on the containment check below.
  if (!/^\d+$/.test(String(timestamp))) {
    return { ok: false, error: 'Sandaran tidak sah' };
  }
  const backupPath = path.join(BACKUP_DIR, String(timestamp));
  // Belt-and-suspenders: even with the numeric-only check above, confirm
  // the resolved path is still actually inside BACKUP_DIR before reading
  // anything from it (same defensive pattern as lib/http-helpers.js's
  // serveStatic - never trust a path built from user input without also
  // checking where it actually landed).
