...
function runChecks() {
  ...
  if (raceStatus && typeof raceStatus === 'object' && !Array.isArray(raceStatus)) {
    for (const [categoryCode, entry] of Object.entries(raceStatus)) {
      if (!entry) continue;
      if (entry.finishedAt && !entry.startTime) {
        warn(`Keadaan perlumbaan tidak sah untuk kategori "${categoryCode}": ada masa tamat tetapi tiada masa mula`);
      }
      if (entry.startTime && entry.finishedAt && entry.finishedAt < entry.startTime) {
        warn(`Keadaan perlumbaan tidak sah untuk kategori "${categoryCode}": masa tamat lebih awal daripada masa mula`);
      }
    }
  }
